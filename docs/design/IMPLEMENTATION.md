...
| 8 | F1 PD6 | **Forward-only data migrations.** `server/persistence/migrate.ts` — `migrateState` walks a loaded snapshot one version at a time up to `STORE_VERSION` (a gap or a newer-than-build snapshot ⇒ null ⇒ reseed); both backends route `load()` through it, replacing the version-mismatch discard. Registry is empty today (no `>v4`); the next `STORE_VERSION` bump appends one migration and existing stores upgrade in place. Locked by `tests/persist-migrate.test.ts` (engine: passthrough/garbage/newer/chain/gap; **+ a SQLite backend integration** proving an older-version store upgrades via a registered migration during `load()`). | ✅ built |
