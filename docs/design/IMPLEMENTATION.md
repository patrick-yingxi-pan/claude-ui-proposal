...
| 17 | F5 | **Baseline security headers.** `SECURITY_HEADERS` (nosniff, `X-Frame-Options: DENY`, CSP `frame-ancestors 'none'`, `Referrer-Policy: no-referrer`) on every API response (success + error, via the shared `BASE_HEADERS`) and on the served UI/asset responses (`server/index.ts`). Locked by `tests/security-headers.test.ts`. | ✅ built |
...
| 19 | F6 | **Graceful-shutdown draining.** SIGTERM/SIGINT flips a drain latch (`store.beginDraining`); `GET /readyz` then returns 503 `draining` so a load balancer stops routing new traffic while in-flight work finishes, while `/healthz` stays up (no mid-drain hard-kill). Locked by `tests/draining.test.ts`. | ✅ built |
