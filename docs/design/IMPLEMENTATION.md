...
| 13 | F4 | **Stale-runner reaping (liveness TTL).** `RunnerRegistry.reapStale(ttlMs)` marks online runners whose last heartbeat is older than the TTL as offline (durable identity kept), emitting `runner.disconnected`; the run daemon calls it each tick (`RUNNER_TTL_MS`, validate-and-floored). `find()` already filters to online, so routing stops targeting a reaped runner. Locked by `tests/registry.test.ts` (reap past TTL, no-op for offline/fresh, heartbeat rescue). | ✅ built |
| 14 | F4 | **Runner enrollment token.** `POST /runners` (enroll/reconnect) is gated on `RUNNER_ENROLL_TOKEN` when set — `Authorization: Bearer` or `x-runner-token` — the shared-secret stand-in for the mTLS/token runner identity; unset ⇒ open (loopback default, suite unaffected). Locked by `tests/runner-enroll.test.ts`. | ✅ built |
