...
| PORT-8 | Production-infra surface implemented in-repo (build log: [`docs/design/IMPLEMENTATION.md`](../design/IMPLEMENTATION.md)) — identity `GET /me` (F2); ops liveness/readiness/metrics `GET /healthz` + `/readyz` + `/metrics` (F6); per-tenant rate limiting, idempotency-key replay, opt-in cursor pagination, and conditional-GET/ETag on cacheable reads (F3); the audit trail is tenant-scoped (F5) and the model round-trip has a wall-clock timeout → graceful fallback whose per-turn outcome (ok / fallback / aborted / error) is metered into `/metrics` (P5/F6). All additive: the UI contract is unchanged. | `server/identity.ts`, `server/ratelimit.ts`, `server/idempotency.ts`, `server/pagination.ts`, `server/http/respond.ts`, `server/generate.ts`, `server/routes/index.ts` | `tests/identity.test.ts`, `tests/ops.test.ts`, `tests/ratelimit.test.ts`, `tests/idempotency.test.ts`, `tests/pagination.test.ts`, `tests/metrics.test.ts`, `tests/conditional-get.test.ts`, `tests/audit-tenant.test.ts`, `tests/generate-timeout.test.ts`, `tests/generation-outcome.test.ts` | ✅ |
