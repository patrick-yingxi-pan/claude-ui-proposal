...
| PORT-8 | Production-infra surface implemented in-repo (build log: [`docs/design/IMPLEMENTATION.md`](../design/IMPLEMENTATION.md)) — identity `GET /me` (F2); ops liveness/readiness `GET /healthz` + `/readyz` (F6); per-tenant rate limiting, idempotency-key replay, and opt-in cursor pagination on list reads (F3). All additive: the UI contract is unchanged. | `server/identity.ts`, `server/ratelimit.ts`, `server/idempotency.ts`, `server/pagination.ts`, `server/routes/index.ts` | `tests/identity.test.ts`, `tests/ops.test.ts`, `tests/ratelimit.test.ts`, `tests/idempotency.test.ts`, `tests/pagination.test.ts` | ✅ |
