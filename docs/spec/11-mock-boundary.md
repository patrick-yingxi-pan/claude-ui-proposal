...
| MOCK-3 | **Connector / MCP detail** — `/connectors/detail` returns fixture resource lists; MCP servers are a static catalog with no real discovery or tool invocation (the model is given only the built-in resource tools, no `mcp_servers`). | `server/data/connectorDetails.ts`, `server/data/contextOptions.ts` | 🟡 |
