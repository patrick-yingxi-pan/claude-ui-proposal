...
| COMMONS-6 | **Cross-TENANT cooperation (P8 slice 1)** — the D12/D11/D14 cooperation of COMMONS-2..4 lifted across the F2 tenant boundary: an owner marks a Project **shared** (`share-project`, owner-only); a *different* tenant may commission its **own** agent onto it (owner-pays), where a private Project refuses the cross-tenant commission (isolation intact) and no tenant can conscript another's agent; a shared Project is projected to a non-owner **redacted** (owner-scoped session/context fields stripped) and its Contributor list is public **identity-only** (foreign `authority`/`grant`/`reservationId` withheld — the D12 posture stays private); `commissionOwnerTenant` attributes metering to the agent owner (D13), not the Project owner. **Partial** — the runtime owner-pays *metering* wiring (needs a commissioned-execution path), cross-tenant guardian coordination, and role-gated cross-tenant *writes* are follow-ups. | `contract/cowork.ts` (`Project.shared`), `contract/relations.ts` + `contract/graph.ts` (`share-project`), `server/store.ts` (projection + `opDeniedForTenant` carve-out + `listCommissions` + `commissionOwnerTenant`) | `tests/cross-tenant-cooperation.test.ts`, `tests/capability-remote.test.ts` | 🟡 |
